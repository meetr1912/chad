...
`llama-server` serves it from any GGUF on any box that can hold one.
...
1. Serve a model on `:8081` — `llama-server --model <gguf> --host 0.0.0.0 --port 8081
   --ctx-size 65536 --parallel 1 --cache-prompt`.
...
- **The warm-prefix checkpoint is off** under `--backend llama` — the KV lives in the
  server — so the in-container agent is not the in-process agent.
