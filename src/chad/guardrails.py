...
def _rm_hits_protected(command: str) -> bool:
    """Scoped recursive-rm screen: checks EVERY target of every rm in the command."""
...
def is_destructive_bash(command: str) -> bool:
    ...
    target-scoped (see _rm_target_protected) so ordinary scoped deletes like
    `rm -rf /tmp/test-deploy` pass."""
...
# constant so the set is testable.
...
def update_work_flags(name, args, result, did_work, made_edit, unverified_edit):
    ...
    if name in SUBSTANTIVE_TOOLS:
        did_work = True
    landed_edit = (name in ("write", "edit")
                   and result.startswith(("[wrote", "[edited")))
    if landed_edit:
        made_edit = True
...
def nudge_for_no_calls(text, hit_cap, made_edit, unverified_edit, read_only_intent,
    ...
    """Pick the nudge for a step that produced NO tool call, in priority order:
    (1) GARBLE/TRUNCATED — a tool-call attempt that parsed to nothing, or the token
    cap hit mid-thought, so it isn't an answer; (2) BARE STALL — the turn produced
    no tool call and no real content; (3) ANSWERED ON PAPER — produced/described
    code but never applied it; (4) UNVERIFIED EDIT — edited but never ran the
    check. Pure: returns (kind, nudge_text) or (None, None); the caller bumps the
    matching counter and appends the nudge. `kind` is one of 'truncated' /
    'no-edit' / 'unverified-edit'. (open_tool_call is run_turn's
    ...
    # a foreign XML tool-call dialect.
    ...
    elif (open_tool_call or garbled_call) and truncation_nudges < 2:
        if open_tool_call:
    ...
    if (not read_only_intent) and (action_task or has_code) \
        ...
        nudge = ("[you described the change but did not apply it — markdown code "
                 ...
                 "content); to change an existing one use `edit`. "
...
def bash_thrash_nudge(consecutive_failed_bash, thrash_nudges):
    ...
    if consecutive_failed_bash >= 4 and thrash_nudges < 2:
        return ("[several commands in a row have failed and you have not edited any file. "
                ...
                "exists use `rg -n`, not `python -c import`. Take a different "
                "action now.]")
    return None
...
# then repeated one short backtick token ~1,300 times until the 8192-token cap — 225s of dead
...
def degenerate_tail(text: str, tail: int = REPEAT_TAIL_CHARS,
    ...
    signature. ~0.5ms of pure Python; run_turn calls it every 16 tokens, i.e. a few
    times per second against a bandwidth-bound decode, so the cost is noise."""
...
# First line of every progress note (cli.py wraps a relaunched turn's user text as
# `{task}\n\n[{note}]`).
...
def progress_note(messages, max_lines: int = 24, rejected_claim: str | None = None) -> str:
    ...
    the stuck model was dragging around). Deterministic: pulls the files edited and the
    commands run from the assistant turns' own <tool_call> blocks via parse_tool_calls.
    ...
    no-empty-diff gate), pass the claim text. The note then leads with an explicit
    warning that the claim was rejected — otherwise every fresh relaunch inherits
    "the build is already complete and verified" as its leading fact, re-confirms it,
    and re-dones (observed 6 relaunches in a row with 86% of the wall unused)."""
    from .toolcall_parse import parse_tool_calls
    edited, commands = [], []
    for m in messages:
        if m.get("role") != "assistant":
            continue
        for name, args in parse_tool_calls(m.get("content", "") or ""):
            if name == "bash":
...
# --- auto-continue relaunch + early-finish review -----------------------
...
def replenish_continue(total_wall_s, elapsed_s, used_continues,
    ...
    explicit base allowance only). `relaunch_budget`'s MIN_RELAUNCH_WALL_S floor
    still vetoes a relaunch too close to the deadline. Pure/testable; the caller
    counts usage."""
    if not total_wall_s:
        return False
    if used_continues >= cap:
        return False
    if frac is None:
        frac = AUTO_CONTINUE_REPLENISH_FRAC
    remaining = total_wall_s - elapsed_s
    return remaining > frac * total_wall_s
