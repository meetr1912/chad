"""OAuth client support for hosted HTTP MCP servers.

Hosted connectors that matter to the operator (Linear, Slack, Atlassian, most SaaS)
require **OAuth**, not a static bearer token. HTTP transport with bearer/PAT auth
already exists; this module adds the OAuth path. It is built on the official `mcp`
