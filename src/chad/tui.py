...
# ---------------------------------------------------------------------------
# Pinned todo panel. Pure helpers so the collapse/glyph logic is
...
# ---------------------------------------------------------------------------
# Input completion. The completer wiring is thin; all the logic
...
class TUI:
    def __init__(self, engine: BaseEngine, ctx_limit: int, mode: str = "normal",
        ...
        self._think_capped = 0             # soft think-cap firings this turn
        ...
        # A pinned todo checklist above the status line. Hidden when
    ...
    def _apply_emit(self, kind: str, text: str):
        ...
        if kind == "think":
            ...
            try:
                ...
                pass
            return
        elif kind == "thinkcap":  # soft think-cap fired this turn (no transcript)
            try:
    ...
    def _status_fragments(self):
        ...
        if self._busy:
            ...
            # ✂N: the adaptive soft think-cap trimmed N over-long reasoning runs
            # this turn. Shown only when it actually fired — the cap is off by default, so the
            # default line is byte-identical to before.
    ...
    def _worker(self):
        while not self._shutdown:
            ...
            try:
                ...
                log.exception("turn error")   # full traceback -> session.log
...
The turn's last `ctx` emit already set `_cur_prompt_tokens` to the
