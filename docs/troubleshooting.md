...
chad runs a small model on your laptop, so a session can ramble, loop, or slow down in ways
a frontier API won't. In a shell-first harness most of what goes wrong is **memory**,
**scope**, or **something you configured that chad deliberately did not auto-enable**. Every
knob that fixes those already exists, and this page maps what you *see* to the one to reach
for. The knobs themselves are documented in the [Configuration
reference](configuration.md); this is the symptom index.
...
| chad refuses to start: `CHAD_DISABLE names unregistered lever(s)` | a typo'd lever name; this is a hard error on purpose, since it would otherwise run the unmodified harness and report a delta of zero | `chad levers` prints the valid names (it loads no model) |
...
lands; "improve my codebase" flails. That is why the harness is shaped the way it is: five
tools, a shell the model already knows, and a context window worth being frugal with. When in
doubt, shrink the task and name the file.
