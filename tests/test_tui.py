...
def test_settle_ctx_gauge_avoids_retokenizing():
    # At end of turn the ctx gauge is refreshed from the last `ctx`
...
def test_todos_emit_updates_panel_not_scrollback():
    # The `todos` kind feeds the pinned panel (parsed JSON list) and,
...
def test_thinkcap_emit_updates_state_not_scrollback():
    # The `thinkcap` kind carries the per-turn soft think-cap count for the
...
def test_thinkcap_glyph_absent_by_default_present_when_capped():
    # The ✂N indicator appears in the busy status line ONLY when the soft
...
def test_first_task_hint_shown_on_fresh_session():
    # A fresh session (resume is None) gets exactly one muted hint line under
...
# ---------------------------------------------------------------------------
# Worker-thread concurrency: queue order, interrupt delivery, the
